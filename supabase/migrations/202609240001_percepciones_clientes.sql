-- Percepciones automáticas de clientes en ventas.
-- Solo aplican a Responsables Inscriptos y se calculan sobre el neto gravado.

begin;

alter table public.cliente
  add column if not exists aplica_percepcion_iva boolean not null default false,
  add column if not exists aplica_percepcion_iibb boolean not null default false;

-- Normaliza cualquier dato previo antes de instalar la regla definitiva.
update public.cliente
set aplica_percepcion_iva = false,
    aplica_percepcion_iibb = false
where coalesce(condicion_fiscal, '') <> 'Responsable Inscripto';

alter table public.cliente
  drop constraint if exists cliente_percepciones_responsable_inscripto_check;

alter table public.cliente
  add constraint cliente_percepciones_responsable_inscripto_check
  check (
    (not aplica_percepcion_iva and not aplica_percepcion_iibb)
    or coalesce(condicion_fiscal, '') = 'Responsable Inscripto'
  );

comment on column public.cliente.aplica_percepcion_iva is
  'Indica si corresponde percepción de IVA del 1% sobre el neto gravado.';
comment on column public.cliente.aplica_percepcion_iibb is
  'Indica si corresponde percepción de IIBB del 3,6% sobre el neto gravado.';

create or replace function public.actualizar_cliente_venta(
  ...
  p_aplica_percepcion_iva boolean,
  p_aplica_percepcion_iibb boolean,
  p_estado boolean
)
...
declare
  ...
  v_es_responsable_inscripto boolean;
begin
  if nullif(trim(coalesce(p_nombre, '')), '') is null then
    raise exception 'El nombre o razón social es obligatorio';
  end if;

  if coalesce(p_condicion_fiscal, '') not in ('Consumidor Final', 'Responsable Inscripto', 'Monotributista', 'Exento') then
    raise exception 'La condición fiscal no es válida';
  end if;

  v_es_responsable_inscripto := (p_condicion_fiscal = 'Responsable Inscripto') is true;
  ...
  set nombre = trim(p_nombre),
      ...
      aplica_percepcion_iva = v_es_responsable_inscripto and coalesce(p_aplica_percepcion_iva, false),
      aplica_percepcion_iibb = v_es_responsable_inscripto and coalesce(p_aplica_percepcion_iibb, false),
      estado = coalesce(p_estado, true)
...
revoke all on function public.actualizar_cliente_venta(integer, text, text, text, text, text, boolean, boolean, boolean) from public;
grant execute on function public.actualizar_cliente_venta(integer, text, text, text, text, text, boolean, boolean, boolean) to anon, authenticated;

-- Se conserva la firma para no romper el frontend ni los borradores existentes.
-- Los parámetros p_percepcion_* quedan solo por compatibilidad: los importes
-- definitivos se calculan en la base según la ficha del cliente.
...
declare
  ...
  v_base_percepciones numeric(14, 2) := 0;
  v_percepcion_iva numeric(14, 2) := 0;
  v_percepcion_iibb numeric(14, 2) := 0;
  v_total numeric(14, 2);
  v_punto_venta integer;
  v_numero integer;
  v_medio_nombre text;
  v_medio_codigo text;
  v_condicion_fiscal text := 'Consumidor Final';
  v_aplica_percepcion_iva boolean := false;
  v_aplica_percepcion_iibb boolean := false;
begin
  ...
  if p_id_cliente is not null then
    select c.condicion_fiscal,
           c.aplica_percepcion_iva,
           c.aplica_percepcion_iibb
      into v_condicion_fiscal,
           v_aplica_percepcion_iva,
           v_aplica_percepcion_iibb
  ...
  -- Valida y bloquea todo el inventario antes de efectuar escrituras.
  ...
  v_base_percepciones := round(v_neto_21 + v_neto_105, 2);
  if v_condicion_fiscal = 'Responsable Inscripto' then
    if v_aplica_percepcion_iva then
      v_percepcion_iva := round(v_base_percepciones * 0.01, 2);
    end if;
    if v_aplica_percepcion_iibb then
      v_percepcion_iibb := round(v_base_percepciones * 0.036, 2);
    end if;
  end if;

  v_total := round(v_total_productos + v_percepcion_iva + v_percepcion_iibb, 2);
  ...
  select s.punto_venta into v_punto_venta
  ...
  perform pg_advisory_xact_lock(1000000 + v_punto_venta);
  ...
  insert into public.venta (
    ...
    p_id_sucursal, p_id_deposito, p_id_cliente, upper(p_tipo_comprobante),
    ...
    v_percepcion_iva, v_percepcion_iibb,
    round(v_percepcion_iva + v_percepcion_iibb, 2),
