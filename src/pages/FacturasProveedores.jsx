...
export default function FacturasProveedores() {
  ...
  const total = ['subtotal', 'iva', 'conceptos_exentos', 'conceptos_no_gravados']
  ...
  async function handleSubmit(event) {
    ...
    const result = await createFacturaProveedor({ ...form, importe_total: total });
    if (result.error) {
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '0.75rem', padding: '1.25rem', marginBottom: '1.5rem' }}>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: '1rem', marginBottom: '1rem' }}>
            ...
            <div><label style={labelStyle}>Orden de compra</label><select value={form.id_orden_compra} onChange={(e) => updateField('id_orden_compra', e.target.value)} style={inputStyle} disabled={!form.id_proveedor}><option value="">Sin asociación</option>{ordenesDisponibles.map((orden) => <option key={orden.id_orden_compra} value={orden.id_orden_compra}>{orden.numero_orden}</option>)}</select></div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, minmax(0, 1fr))', gap: '1rem', alignItems: 'end' }}>
            {['subtotal', 'iva', 'conceptos_exentos', 'conceptos_no_gravados'].map((field) => <div key={field}><label style={labelStyle}>{field === 'conceptos_exentos' ? 'Exentos' : field === 'conceptos_no_gravados' ? 'No gravados' : field.toUpperCase()} {field === 'subtotal' || field === 'iva' ? '*' : ''}</label><input type="number" min="0" step="0.01" value={form[field]} onChange={(e) => updateField(field, e.target.value)} style={inputStyle} required={field === 'subtotal' || field === 'iva'} /></div>)}
            <div style={{ background: '#f7f3ef', borderRadius: '0.5rem', padding: '0.625rem 0.75rem', minHeight: '42px', boxSizing: 'border-box' }}><span style={{ ...labelStyle, marginBottom: '0.15rem' }}>Total</span><strong style={{ color: '#65482b', fontSize: '1.1rem' }}>{currency(total)}</strong></div>
          </div>
